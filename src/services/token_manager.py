...
class TokenManager:
    ...
    # ========== Token Add (with Project creation support) ==========
    ...
    async def update_token(
        ...
        # Check if token was banned due to 429, if so and not expired, clear 429 status
    ...
    def _should_refresh_at(self, token: Token) -> bool:
        ...
        if not token.at:
            debug_logger.log_info(f"[AT_CHECK] Token {token.id}: AT not found, needs refresh")
            return True
        ...
        if time_until_expiry.total_seconds() < 3600:
            debug_logger.log_info(
                ...
                f"({time_until_expiry.total_seconds():.0f}s remaining), needs refresh"
            )
            return True
    ...
    async def ensure_valid_token(self, token: Optional[Token]) -> Optional[Token]:
        """Ensure token AT is available, return refreshed latest object if needed."""
        if not token:
            return None
    ...
    async def is_at_valid(self, token_id: int, token: Optional[Token] = None) -> bool:
        """Check if AT is valid, auto-refresh if invalid or expiring soon
    ...
    async def _do_refresh_at(self, token_id: int, st: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                debug_logger.log_info(f"[AT_REFRESH] Token {token_id}: AT verified successfully (balance: {credits_result.get('credits', 0)})")
                return True
            except Exception as verify_err:
                # AT verification failed (may return 401), ST may be expired
                error_msg = str(verify_err)
                if "401" in error_msg or "UNAUTHENTICATED" in error_msg:
                    debug_logger.log_warning(f"[AT_REFRESH] Token {token_id}: AT verification failed (401), ST may be expired")
                    return False
                else:
                    # Other errors (e.g., network issues), still treat as success
    ...
    async def _try_refresh_st(self, token_id: int, token) -> Optional[str]:
        """Attempt to refresh Session Token via browser

        Use resident tab to get new __Secure-next-auth.session-token
        ...
        try:
            ...
            # Only personal mode supports ST auto-refresh
            ...
            debug_logger.log_info(f"[ST_REFRESH] Token {token_id}: Attempting to refresh ST via browser...")
            ...
            if new_st and new_st != token.st:
                ...
                debug_logger.log_info(f"[ST_REFRESH] Token {token_id}: ST has been auto-updated")
                return new_st
            elif new_st == token.st:
                debug_logger.log_warning(f"[ST_REFRESH] Token {token_id}: ST same as original, login may be invalid")
                return None
            else:
                debug_logger.log_warning(f"[ST_REFRESH] Token {token_id}: Cannot get new ST")
                return None
    ...
    async def ban_token_for_429(self, token_id: int):
        """Immediately ban token due to 429 error
    ...
    async def auto_unban_429_tokens(self):
        """Auto unban tokens banned due to 429
        ...
        for token in all_tokens:
            # Skip tokens not banned for 429
            if token.ban_reason != "429_rate_limit":
                continue

            # Skip non-banned tokens
            if token.is_active:
                continue
